...
def read_image(fn):
    im = cv2.imread(fn,-1)
    im = np.reshape(im,(imageSize,imageSize,1))
    arr = im/750.0-1.0
...
data = "mito"
train_A = load_data('CycleGAN/trainA/*.tif')
train_B = load_data('CycleGAN/trainB/*.tif')
...
def showX(X, rows=1, save_file=None):
    ...
    if channel_first:
        int_X = np.moveaxis(int_X.reshape(-1,3,imageSize,imageSize), 1, 1)
    else:
...
while epoch < niter: 
    ...
    errGA_all.append(errGA)
    errGB_all.append(errGB)
    errDA_all.append(errDA)
    errDB_all.append(errDB)
    errCyc_all.append(errCyc)
    ...
    if gen_iterations%display_iters==0:
        ...
        # save_file = 'results/val_epoch%d.png' % epoch
        save_file = None
        showG(A,B,save_file)        
        errCyc_sum = errGA_sum = errGB_sum = errDA_sum = errDB_sum = 0
    if epoch%25 == 0:
        netGA.save('temp_results/netGA_epoch%d.h5' % epoch)
        netGB.save('temp_results/netGB_epoch%d.h5' % epoch)
        netDA.save('temp_results/netDA_epoch%d.h5' % epoch)
        netDB.save('temp_results/netDB_epoch%d.h5' % epoch)
...
plt.figure(1)
plt.plot(range(gen_iterations),errGA_all)
plt.title("errGA")
plt.savefig("errGA.png")

plt.figure(2)
plt.plot(range(gen_iterations),errGB_all)
plt.title("errGB")
plt.savefig("errGB.png")

plt.figure(3)
plt.plot(range(gen_iterations),errDA_all)
plt.title("errDA")
plt.savefig("errDA.png")

plt.figure(4)
plt.plot(range(gen_iterations),errDB_all)
plt.title("errDB")
plt.savefig("errDB.png")

plt.figure(5)
plt.plot(range(gen_iterations),errCyc_all)
plt.title("errCyc")
plt.savefig("errCyc.png")

np.save('errGA.npy',errGA_all)
np.save('errGB.npy',errGB_all)
np.save('errDA.npy',errDA_all)
np.save('errDB.npy',errDB_all)
np.save('errCyc.npy',errCyc_all)
